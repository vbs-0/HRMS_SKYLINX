...
export default function RemindersPage() {
  ...
  return (
    <>
      ...
      {/* Add Rule Form */}
      {showAdd && (
        <div className="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm animate-in zoom-in-95 duration-150">
          ...
          <form onSubmit={handleAddRule} className="grid grid-cols-3 gap-4 max-md:grid-cols-1">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Days Offset (before event)</label>
              <input
                name="daysOffset"
                type="number"
                defaultValue={7}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Email Subject</label>
              <input
            ...
            <div className="col-span-2 max-md:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 mb-1">Email Body</label>
              <input
                name="templateBody"
      ...
      {rules.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-200 py-16 text-center">
          ...
          <p className="text-sm font-semibold text-slate-500">No automation rules configured yet.</p>
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <button
                  ...
                  className="flex-shrink-0 text-slate-400 hover:text-brand transition"
              ...
              <div className="space-y-1.5 text-xs text-slate-500">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-700">{rule.daysOffset}d before</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-slate-400">Channel</span>
                  <span
                    className={`rounded px-1.5 py-0.5 text-xs font-bold uppercase ${
                      CHANNEL_COLORS[rule.channel] ?? "bg-slate-100 text-slate-600"
                    }`}
                  >
                    {rule.channel}
                  </span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-700 truncate max-w-[140px]" title={rule.templateSubject}>
