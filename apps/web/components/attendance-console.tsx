  ...
  return (
    <>
      ...
      {activeTab === "Attendance Log" && (
        <Card>
          <h2 className="mb-4 text-lg font-semibold text-text-primary text-left">Today&apos;s Logs</h2>
  ...
  return (
    <Card className="p-5 border border-[var(--border-subtle)]">
      ...
      <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-end max-md:grid-cols-1">
        <div>
          <label className="block text-xs font-semibold text-text-secondary mb-1" htmlFor="geo-employee-select">
            Employee
          </label>
          <select
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-xs font-bold transition ${
            geoStatus === "ready"
              ...
              : "bg-sunken text-text-primary border border-line hover:bg-surface-hover"
          }`}
        >
      ...
      {geoError && (
        <p className="mt-2 text-xs text-danger-fg">{geoError}</p>
      )}
  ...
  return (
    <div className="grid gap-6 text-left">
      ...
      <div className="grid grid-cols-[1fr_2fr] gap-6 max-lg:grid-cols-1">
        <Card className="p-5 border border-[var(--border-subtle)]">
          <h3 className="text-base font-bold text-text-primary mb-4 border-b pb-2">Bulk Attendance Upload</h3>
          <p className="text-xs text-text-muted mb-4 font-normal">Upload CSV sheet to parse multiple attendance records at once.</p>
          <form onSubmit={handleUpload} className="grid gap-4">
            <div className="rounded-lg border-2 border-dashed border-line p-8 text-center hover:bg-sunken transition cursor-pointer relative">
              <input type="file" name="file" accept=".csv" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" required />
              <div className="text-sm font-semibold text-text-secondary">Click or drag CSV file to upload</div>
            </div>
            <button
              type="submit"
              disabled={submitting}
        ...
        <Card className="p-5 border border-[var(--border-subtle)] bg-[var(--surface-sunken)]">
          <h4 className="text-sm font-bold text-text-primary mb-2">CSV Format Expected:</h4>
          <p className="text-xs text-text-secondary mb-4">Your CSV must include the following headers exactly as shown:</p>
          <ul className="list-disc pl-5 text-xs text-text-secondary font-mono space-y-1 mb-4">
            <li>employeeId</li>
            <li>date</li>
            <li>checkInAt</li>
            <li>checkOutAt</li>
