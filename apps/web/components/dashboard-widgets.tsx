  ...
  return (
    <div className="grid content-start gap-4">
      ...
      {celebrations.length > 0 && (
        <div className="rounded-lg border border-warning-border bg-warning-bg p-4 shadow-sm">
          <h3 className="flex items-center gap-1.5 text-sm font-bold text-warning-fg mb-3">
            <PartyPopper className="h-4 w-4" /> Today&apos;s Celebrations
          </h3>
          <div className="space-y-2">
            {celebrations.map((c, i) => (
              <div key={i} className="flex items-start gap-2 text-xs text-warning-fg">
                <span className="text-base leading-tight">{typeIcon[c.type] || "🎉"}</span>
                <div>
                  <span className="font-semibold">{c.employeeName}</span>
                  <span className="text-warning-fg ml-1">— {c.message}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {announcements.map((ann) => (
              <div key={ann.id} className="border-l-2 border-blue-400 pl-2">
                ...
                <p className="text-[10px] text-text-secondary mt-0.5 line-clamp-2">{ann.body}</p>
              </div>
            ))}
          </div>
          <Link href="/notifications" className="mt-3 block text-[10px] text-info-fg hover:underline">
          ...
          <div className="space-y-2">
            {pendingPolicies.map((p) => (
              <div key={p.id} className="flex items-center justify-between text-xs">
                <span className="text-warning-fg font-medium truncate max-w-[160px]">{p.title}</span>
                <span className="text-[10px] bg-orange-200 text-warning-fg px-1.5 py-0.5 rounded font-bold flex-shrink-0 ml-2">
                  {p.category}
                </span>
              </div>
            ))}
          </div>
          <Link href="/policies" className="mt-3 block text-[10px] text-warning-fg font-semibold hover:underline">
            Review & Acknowledge →
          </Link>
        </div>
      )}
