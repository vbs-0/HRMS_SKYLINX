          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {policies.map((policy) => (
              <Card key={policy.id} className="p-5 border border-[var(--border-subtle)] hover:shadow-md transition text-left flex flex-col justify-between">
                <div>
                  <h4 className="text-lg font-bold text-text-primary mb-2">{policy.name}</h4>
                  <p className="text-xs text-text-secondary mb-4">{policy.description || "No description provided."}</p>
                  <hr className="border-line my-3" />
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted">Assigned Employees</span>
                    <div className="max-h-36 overflow-y-auto space-y-1 mt-1.5">
                      {policy.assignments && policy.assignments.length > 0 ? (
                        policy.assignments.map((as) => (
                          <div key={as.id} className="text-xs font-semibold text-text-primary bg-sunken border px-2 py-1 rounded">
                            {as.employee.firstName} {as.employee.lastName}
                          </div>
                        ))
                      ) : (
                        <div className="text-xs text-text-muted font-medium italic">No employees assigned yet.</div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <Card key={bl.id} className="p-5 border border-[var(--border-subtle)] hover:shadow-md transition text-left flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="text-lg font-bold text-text-primary">{bl.name}</h4>
                    <button
                  ...
                  <p className="text-xs text-text-secondary mb-4">{bl.description || "No description provided."}</p>
                  <hr className="border-line my-3" />
                  <div className="space-y-2">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted">Blocked Dates & Reasons</span>
                    <div className="max-h-48 overflow-y-auto space-y-1.5 mt-1.5">
                      {bl.dates && bl.dates.length > 0 ? (
                        bl.dates.map((d) => (
                          <div key={d.id} className="text-xs border p-2 rounded bg-danger-bg border-danger-border flex items-start gap-2">
                            <Calendar className="h-4 w-4 text-danger-fg mt-0.5 flex-shrink-0" />
                            <div>
                              <div className="font-bold text-text-primary">{new Date(d.date).toLocaleDateString()}</div>
                              <div className="text-text-secondary mt-0.5 font-medium">{d.reason}</div>
                            </div>
                          </div>
                        ))
                      ) : (
                        <div className="text-xs text-text-muted font-medium italic">No blocked dates added yet.</div>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-raised p-6 shadow-2xl text-left">
            ...
            <form onSubmit={handleCreatePolicy} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Policy Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full min-h-10 rounded-lg border border-line px-3 text-sm focus:border-brand outline-none"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full min-h-16 rounded-lg border border-line p-2.5 text-sm focus:border-brand outline-none"
                />
              </div>
              <div className="flex justify-end gap-2 pt-2 border-t">
                <button
                  ...
                  className="min-h-10 rounded-lg border border-line px-4 text-xs font-bold text-text-primary hover:bg-sunken transition"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-raised p-6 shadow-2xl text-left">
            ...
            <form onSubmit={handleAssignPolicy} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Select Policy</label>
                <select
                  required
              ...
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Select Employees</label>
                <select
                  multiple
                  required
                ...
                <span className="text-[10px] text-text-muted block mt-1">Hold Ctrl (Windows) or Cmd (Mac) to select multiple employees.</span>
              </div>
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Effective From Date</label>
                <input
                  type="date"
                  required
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-raised p-6 shadow-2xl text-left">
            ...
            <form onSubmit={handleCreateBlockList} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Block List Name</label>
                <input
                  type="text"
                  required
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-raised p-6 shadow-2xl text-left">
            ...
            <form onSubmit={handleAddBlockDate} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Date to Block</label>
                <input
                  type="date"
                  required
              ...
              <div>
                <label className="text-xs font-semibold text-text-secondary block mb-1">Reason for Block</label>
                <input
                  type="text"
                  required
