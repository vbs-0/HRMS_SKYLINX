      ...
      {activeTab === "Onboarding" && (
        <div className="grid grid-cols-2 gap-6 max-lg:grid-cols-1 text-left">
          ...
          <div className="grid gap-6">
            ...
            <Card>
              <h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-text-primary">Existing Templates</h3>
              {onbTemplates.length === 0 ? (
                <p className="text-xs text-text-muted">No onboarding templates defined yet.</p>
              ) : (
                <div className="grid gap-3">
                  {onbTemplates.map((t) => (
                    <div key={t.id} className="rounded-lg border border-line p-3 bg-sunken">
                      <div className="font-semibold text-text-primary">{t.name}</div>
                      <div className="mt-2 text-xs text-text-secondary">
                        {t.activities?.length} Activities:
                      </div>
                      <div className="mt-1 pl-3 text-xs text-text-muted list-disc grid gap-1">
            ...
            <form onSubmit={handleCreateOnbTemplate} className="grid gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-bold uppercase text-text-secondary">Activities & Tasks</label>
                  <button
                    type="button"
                ...
                <div className="grid gap-3">
                  {onbActivities.map((act, idx) => (
                    <div key={idx} className="grid gap-2 border border-line rounded-lg p-3 bg-sunken relative">
                      ...
                      <div>
                        <input
                          className="min-h-9 w-full rounded-md border border-line px-3 text-xs"
        ...
        <div className="grid grid-cols-2 gap-6 max-lg:grid-cols-1 text-left">
          ...
          <div className="grid gap-6">
            ...
            <Card>
              ...
              {sepTemplates.length === 0 ? (
                <p className="text-xs text-text-muted">No separation templates defined yet.</p>
              ) : (
                <div className="grid gap-3">
                  {sepTemplates.map((t) => (
                    <div key={t.id} className="rounded-lg border border-line p-3 bg-sunken">
          ...
          <Card>
            ...
            <form onSubmit={handleCreateSepTemplate} className="grid gap-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-bold uppercase text-text-secondary">Clearance Checklist Items</label>
                  <button
                    type="button"
            ...
            <Card>
              ...
              <form onSubmit={handleExitInterviewSubmit} className="grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Exit Date</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-sm"
                      type="date"
                      value={exitDate}
                ...
                <div>
                  <label className="block text-xs font-semibold text-text-secondary mb-1">Reason for Leaving</label>
                  <input
                    className="min-h-10 w-full rounded-lg border border-line px-3 text-sm"
                    value={reason}
                ...
                <div>
                  <label className="block text-xs font-semibold text-text-secondary mb-1">Feedback / Notes</label>
                  <textarea
                    className="w-full rounded-lg border border-line p-3 text-sm h-20"
                    value={feedback}
            ...
            <Card>
              ...
              <form onSubmit={handleCalculateFnF} className="grid gap-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Last Drawn Salary (Base)</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-sm"
                      type="number"
                      value={ffSalary}
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Resignation Date</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Last Working Date</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="date"
                      value={ffExitDate}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Notice Days</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                      value={ffNoticeDays}
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Gratuity (Estimated)</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                      value={ffGratuity}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Leave Encashment Dues</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                      value={ffEncashment}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Unpaid Salary Dues</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Notice Shortfall Pay Deduction</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1">Other Recoveries & Deductions</label>
                    <input
                      className="min-h-10 w-full rounded-lg border border-line px-3 text-xs"
                      type="number"
                      value={ffRecovery}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-xs font-bold uppercase text-text-secondary">Asset Clearances</label>
                    <button
                      type="button"
                      onClick={addFfAsset}
                      className="inline-flex items-center gap-1 text-xs font-semibold text-brand hover:underline"
                  ...
                  <div className="grid gap-2">
                    {ffAssets.map((asset, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        <input
                          className="min-h-9 flex-1 rounded-md border border-line px-3 text-xs"
                        ...
                        />
                        <input
                          className="min-h-9 w-24 rounded-md border border-line px-3 text-xs"
                          type="number"
          ...
          <div>
            {statement ? (
              <Card className="border border-[var(--border-subtle)]">
                <h3 className="mb-4 text-sm font-bold uppercase tracking-wider text-text-primary border-b pb-3 flex items-center justify-between">
                  <span>Generated Full & Final Statement</span>
                  <span className="rounded bg-success-bg px-2 py-1 text-xs font-bold text-success-fg uppercase tracking-wide">
                    {statement.status}
                  </span>
                </h3>

                <div className="grid gap-3 text-sm mb-5">
                  <div className="grid grid-cols-2 gap-4 bg-sunken p-3 rounded-lg">
                    <div>
                      <span className="text-[11px] font-bold uppercase text-text-muted">Resignation Date</span>
                      <div className="font-semibold text-text-primary">{statement.resignationDate?.slice(0, 10)}</div>
                    </div>
                    <div>
                      <span className="text-[11px] font-bold uppercase text-text-muted">Last Working Date</span>
                      <div className="font-semibold text-text-primary">{statement.exitDate?.slice(0, 10)}</div>
                    </div>
                  </div>

                  <div className="border-t border-line pt-3 grid gap-2">
                    <div className="flex justify-between border-b pb-2">
                      <span className="text-text-secondary">Last Drawn Salary (Dues)</span>
                      <span className="font-semibold text-text-primary">INR {Number(statement.lastDrawnSalary).toLocaleString("en-IN")}</span>
                    </div>
                    <div className="flex justify-between border-b pb-2">
                      <span className="text-text-secondary">Gratuity Payout</span>
                      <span className="font-semibold text-text-primary">INR {Number(statement.gratuityDues).toLocaleString("en-IN")}</span>
                    </div>
                     <div className="flex justify-between border-b pb-2">
                      <span className="text-text-secondary">Leave Encashment</span>
                      <span className="font-semibold text-text-primary">INR {Number(statement.encashmentDues).toLocaleString("en-IN")}</span>
                    </div>
                    {Number(statement.unpaidSalary || 0) > 0 && (
                      <div className="flex justify-between border-b pb-2">
                        <span className="text-text-secondary">Unpaid Salary Dues</span>
                        <span className="font-semibold text-text-primary">INR {Number(statement.unpaidSalary).toLocaleString("en-IN")}</span>
                      </div>
                    )}
                    {Number(statement.noticeShortfall || 0) > 0 && (
                      <div className="flex justify-between border-b pb-2 text-danger-fg">
                ...
                <div className="border-t pt-4">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-text-secondary mb-3">Asset Checklist Recovery</h4>
                  {statement.assets?.length === 0 ? (
                    <p className="text-xs text-text-muted">No assets mapped to this statement.</p>
                  ) : (
                    <div className="grid gap-3">
                      {statement.assets.map((asset) => (
                        <div key={asset.id} className="flex justify-between items-center rounded-lg border p-3">
                          <div>
                            <div className="font-semibold text-text-primary text-xs">{asset.assetName}</div>
                          ...
                          <div className="flex gap-1">
                            <button
                              type="button"
                              className={`px-2 py-1 text-[10px] font-bold rounded ${asset.returnedStatus === "RETURNED" ? "bg-success-bg text-success-fg" : "bg-sunken text-text-secondary"}`}
                            ...
                            <button
                              type="button"
                              className={`px-2 py-1 text-[10px] font-bold rounded ${asset.returnedStatus === "RECOVER_COST" ? "bg-danger-bg text-danger-fg" : "bg-sunken text-text-secondary"}`}
                            ...
                            <button
                              type="button"
                              className={`px-2 py-1 text-[10px] font-bold rounded ${asset.returnedStatus === "PENDING" ? "bg-warning-bg text-warning-fg" : "bg-sunken text-text-secondary"}`}
              ...
              <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-line rounded-xl p-8 bg-sunken text-text-muted min-h-[300px]">
                <CheckSquare className="h-10 w-10 text-text-muted mb-2" />
                <div className="text-sm font-semibold">No Statement Loaded</div>
                <p className="text-xs text-text-muted max-w-xs text-center mt-1">
