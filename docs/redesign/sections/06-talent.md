...
- **Team Appraisals** (manager): Grade Report (per-KRA rating + comment) when SELF_DONE → MANAGER_DONE. EXISTS `POST …/:id/manager-rate` (enforces `managerId===caller`). **Fix seed (critique T-2):** grant `performance.approve` to managers **and to HR_ADMIN** (HR holds neither `performance.approve` nor `performance.create` today, rbac A3), and `performance.read` to employees — the whole self→manager→HR flow is otherwise unreachable; **fix** team filter to server-side reports-only (today client-side shows all non-self, talent.md §1.4).
...
- **Programs & Events**: create program/event (program, eventName, trainer, start/end, location). **Fix (critique T-1):** `TrainingEvent.status` **does exist** (`@default("SCHEDULED")`) — the inventory's "no status field/renders undefined" was wrong; the real gaps are (a) status is a free string (no enum) and (b) there's **no lifecycle PATCH** (edit/cancel/complete). **NEW**: event edit/cancel/complete (no PATCH/DELETE today); **NEW**: enrollment/attendee concept + nominations (manager nominate → `training.approve` — **fix** the dead MANAGER grant, talent.md §3.2). EXISTS `POST/GET /training/programs|events`.
...
**Title** "Travel Desk" · **Plan** Standard. **Roles** `travel.{read,create,update,approve}`. **Fix seed (critique T-9, talent.md §4.1):** `travel.read` is HR-only and `travel.approve` is MANAGER+SUPER_ADMIN-only → **HR_ADMIN cannot approve travel at all**, and seeded EMPLOYEE/MANAGER consoles error on read. Grant `travel.approve` to HR_ADMIN and self/team-scoped `travel.read` to employees/managers.
